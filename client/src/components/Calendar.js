...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    const searchMatches = document.querySelectorAll(
      `.${searchStyles.searchMatch}`
    );
  ...
  const handleEventClick = (info) => {
    ...
    let displayColor =
      currEvent.color !== undefined ? currEvent.color : "#000000";
    let newDisplayInfo = {
  ...
  const showMedHeight = useMediaQuery(
    "(min-height:380px) and (max-height:500px)"
  );
    ...
    <div>
      <Dialog open={open} onClose={handleClose}>
        <Box sx={{ padding: "30px" }}>
          ...
          >
            {displayInfo.societyName}
          </h3>
          <h6 style={{ marginTop: "0px", marginBottom: "2px" }}>
            <b>
      ...
      {/* ABOVE 1024PX */}
      {showBig && !showMedHeight && !showSmallHeight && (
          ...
          <Grid item xs={2}>
            <div className={styles.socWrapper}>
              <SearchBar
                ...
                fullSocList={fullSocList}
                delSociety={delSociety}
              />
              <SocFollowing
                societies={fullSocList}
                addSociety={addSociety}
                delSociety={delSociety}
              />
            </div>
      ...
      {(showMedium || showMedHeight) && !showSmallHeight && (
        <Grid container spacing={2} sx={{ padding: "15px" }}>
          <Grid item xs={12}>
            <FullCalendar
              initialView="dayGridMonth"
      ...
      {/* ANYTHING SMALLER */}
      {(showSmall || showSmallHeight) && !showMedHeight && (
        <Grid container spacing={2} sx={{ padding: "15px" }}>
          <Grid item xs={12}>
            <FullCalendar
              initialView="dayGridMonth"
